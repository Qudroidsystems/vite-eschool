...
<div class="header">
    @if($schoolInfo && $schoolInfo->school_logo)
        <img src="{{ $schoolInfo->logo_url }}" alt="School Logo" class="school-logo">
    @endif
    <div class="school-name">{{ $schoolInfo->school_name ?? 'TOPCLASS COLLEGE' }}</div>
    @if($schoolInfo)
        @if($schoolInfo->school_address)<div class="school-details">{{ $schoolInfo->school_address }}</div>@endif
        @if($schoolInfo->school_phone)<div class="school-details">Tel: {{ $schoolInfo->school_phone }}</div>@endif
        @if($schoolInfo->school_email)<div class="school-details">Email: {{ $schoolInfo->school_email }}</div>@endif
        @if($schoolInfo->school_motto)<div class="school-details"><em>"{{ $schoolInfo->school_motto }}"</em></div>@endif
    @endif
</div>
...
<table class="marks-table">
    ...
    <tbody>
        ...
        @empty
            @for($i = 1; $i <= 25; $i++)
                <tr>
                    <td>{{ $i }}</td><td></td><td class="student-name"></td><td></td><td></td><td></td><td></td>
                </tr>
            @endfor
        @endforelse
    </tbody>
