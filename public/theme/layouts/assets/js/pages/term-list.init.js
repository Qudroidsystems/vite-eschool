...
try {
    ...
    var termList = new List('termList', {
    ...
    function filterData() {
        ...
        termList.search(searchValue, ['term']); // Search in term column
    }
    ...
    if (addTermForm) {
        addTermForm.addEventListener("submit", function (e) {
            ...
            if (!addTermField || !addTermField.value) {
                if (errorMsg) errorMsg.innerHTML = "Please enter a term name";
                return false;
            }

            axios.post('/term', {
                term: addTermField.value,
    ...
    if (editTermForm) {
        editTermForm.addEventListener("submit", function (e) {
            ...
            axios.put(`/term/${editIdField.value}`, {
                term: editTermField.value,
                _token: document.querySelector('meta[name="csrf-token"]').content
            }).then(function (response) {
                Swal.fire({
                    position: "center",
    ...
    console.error("Error in term.init.js:", error);
}