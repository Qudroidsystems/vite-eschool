...
function registerSelectedStudents() {
    ...
    subjectClasses.forEach(subject => {
        ...
        promises.push(
            axios.post('/subjectregistration', data, {
                headers: { 
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                }
            })
    ...
    Promise.allSettled(promises).then(function (results) {
        ...
        results.forEach((result, index) => {
            if (result.status === 'fulfilled') {
                ...
                const subject = subjectClasses[index];
                errorMessages.push(`Subjectclass ID ${subject.subjectclassid}, Term ID ${subject.termid}: ${result.reason?.response?.data?.message || 'Unknown error'}`);
            }
        });
        ...
        if (successCount === promises.length) {
            Swal.fire({
                icon: 'success',
                title: 'Success',
                text: `Successfully registered ${successCount} subject(s) for ${studentIds.length} student(s).`,
                showConfirmButton: true
            });
        } else {
            Swal.fire({
                ...
                html: `Successfully registered ${successCount} subject(s). Errors:<br>${errorMessages.join('<br>')}`,
                showConfirmButton: true
            });
        }
...
function loadRegisteredClasses() {
    ...
    console.log('All select elements:', Array.from(selectElements).map(sel => ({
        id: sel.id,
        name: sel.name,
    ...
    const classOptions = Array.from(classSelect.options).map(opt => ({ value: opt.value, text: opt.text }));
    const sessionOptions = Array.from(sessionSelect.options).map(opt => ({ value: opt.value, text: opt.text }));
    console.log('Dropdown options:', { classOptions, sessionOptions });

    const classId = classSelect.value;
    const sessionId = sessionSelect.value;

    console.log('Selected values:', { classId, sessionId });
